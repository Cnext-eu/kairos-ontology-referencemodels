...
4. **Read the accelerator domain registry** — open the applicable accelerator
   pack's `data-domains.yaml` (e.g.,
   `ontology-reference-models/accelerator-packs/logistics/client-hub-blueprint/data-domains.yaml`).
   Identify the current domain's entry and note its `owns` and `does_not_own`
   boundaries.  These boundaries are **authoritative** — do not model concepts
   that belong to another domain.
5. **Provenance check (mandatory)** — before designing any class or property,
   follow the [Domain Ownership Gate](#domain-ownership-gate) section below to
   verify the concept is not already defined in a reference model or owned by
   another domain.
...
7. **Check the master ontology** — after creating a new domain file, add an
...
> **MANDATORY** — This gate must be passed before defining ANY new `owl:Class`,
> `owl:ObjectProperty`, `owl:DatatypeProperty`, or enum individual.

### Purpose

Prevent modelling concepts that belong to another domain or already exist in a
reference model.  This avoids duplication (e.g., defining `:Container` in
`cargo.ttl` when it belongs to the Equipment domain and is already in MMT/DCSA
reference models).

### Step 1 — Identify ownership boundaries

Read the accelerator blueprint's `data-domains.yaml` for the domain you are
working in.  Note:

- **`owns`** — concepts this domain is responsible for.
- **`does_not_own`** — concepts explicitly excluded (they live elsewhere).

Example for Cargo domain:
```yaml
owns: "Cargo description, goods classification, packages, weights, ..."
does_not_own: "Transport equipment, terminal moves, automotive services, ..."
```

### Step 2 — Search for existing definitions

Before creating a new class or property, search for it:

1. **Blueprint text search** — grep the `data-domains.yaml` `owns` and
   `provides` fields across ALL domains for the concept name and synonyms.
2. **Reference model search** — search
   `ontology-reference-models/derived-ontologies/**/*.ttl` for:
   - Class names (e.g., `Container`, `FreightContainer`)
   - `rdfs:label` values (case-insensitive)
   - Property names (e.g., `containerNumber`, `equipmentIdentifier`)
   - Synonyms and related terms
3. **Blueprint `provides` check** — look at the `provides` text in every
   domain's imports list.

```bash
# Example: searching for "container" across reference models
grep -ri "container" ontology-reference-models/derived-ontologies/ --include="*.ttl"
# Example: searching the blueprint for ownership
grep -i "container" ontology-reference-models/accelerator-packs/logistics/client-hub-blueprint/data-domains.yaml
```

### Step 3 — Apply the decision table

| Concept found where? | Action |
|---|---|
| In a reference model module listed in **current domain's** imports | ✅ Import that module — do NOT redefine the class/property locally |
| In a reference model belonging to **another domain** | 🚫 Do NOT define locally — suggest creating the other domain, or use a cross-domain reference (see below) |
| In reference model but **ownership is unclear** (appears in multiple domains) | ❓ Stop and ask the user which domain should own it |
| **Not found** anywhere AND within current domain's `owns` boundary | ✅ Define locally as a new class/property |
| Within current domain's `does_not_own` list | 🚫 Never define locally — it explicitly belongs elsewhere |

### Step 4 — Cross-domain references (when you need to link, not own)

When the current domain needs to **reference** an entity owned by another domain:

**Loose coupling (preferred — avoids circular imports):**
```turtle
:hasEquipment a owl:ObjectProperty ;
    rdfs:label "has equipment"@en ;
    rdfs:comment "Links to transport equipment (containers, trailers). Owned by Equipment domain."@en ;
    rdfs:domain :ForwardingJob ;
    rdfs:range owl:Thing .
```

**Direct reference (when safe — one-way dependency, no circular imports):**
```turtle
...
**Never do this:**
```turtle
# ❌ WRONG — redefining a concept owned by another domain
:Container a owl:Class ;
    rdfs:label "Container"@en ;
    rdfs:comment "A shipping container."@en .
```

### Worked example — The Container mistake

When creating `cargo.ttl` for a logistics hub:

1. User asks to model cargo with containers.
2. Check `data-domains.yaml` → Cargo domain `does_not_own`: "Transport equipment".
3. Check Equipment domain `owns`: "Trailers, containers, tank containers, reefer units..."
4. Search reference models → found `mmt:FreightContainer` and `dcsa:Container`.
5. **Correct action**: suggest creating the Equipment domain with `owl:imports`
   to `mmt/equipment#` and `dcsa/equipment#`. In `cargo.ttl`, use a cross-domain
   reference property with `rdfs:range owl:Thing`.
6. **Wrong action**: define `:Container`, `:ContainerType`, `:containerNumber` in `cargo.ttl`.
...
- Do NOT create a class or property without first running the
  [Domain Ownership Gate](#domain-ownership-gate) provenance check.
- Do NOT define a class that exists in a reference model — import the module
  instead.  Example: `:Container` belongs in the Equipment domain (MMT/DCSA),
  not in `cargo.ttl`.
- Do NOT assume a concept belongs to the current domain — always verify against
  `data-domains.yaml` `owns` and `does_not_own` fields.
- Do NOT import another domain's reference model and then redefine its classes
  locally — importing for range/reference use does not transfer ownership.
